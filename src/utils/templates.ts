 ...
 */
export async function compileTemplates(projectDir: string): Promise<void> {
  try {
    // Create directory structure in parallel
    await Promise.all([
      fs.ensureDir(path.join(projectDir, "src", "server", "db")),
      fs.ensureDir(path.join(projectDir, "src", "server", "db", "schema")),
    ]);

    // Write all template files in parallel
    await Promise.all([
      writeDrizzleConfig(projectDir),
      writeDbFile(projectDir),
      writeSchemaFiles(projectDir),
      writeMigrationScript(projectDir),
      writeEnvFile(projectDir),
      writeServerActions(projectDir),
      writeExamplePage(projectDir),
      writeSeedScript(projectDir),
    ]);
  } catch (error) {
    throw new Error(
...
async function writeSeedScript(projectDir: string): Promise<void> {
  ...
  // Load environment variables for scripts run outside Next.js
  import "dotenv/config";
